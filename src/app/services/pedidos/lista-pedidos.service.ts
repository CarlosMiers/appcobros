...
export class ListaPedidosService {
  ...
  constructor() {
    ...
    this.myApiUrlActivo = '/preventa/activos';
    this.myApId = 'id=';
  ...
  async getListaPedidosActivos(): Promise<ListaPedido> {
    const token = localStorage.getItem('token'); // Recuperamos el token desde localStorage
    ...
    const url = `${this.base_path}${this.myApiUrlActivo}`; // Se eliminó la variable 'myApId' y el 'id'
