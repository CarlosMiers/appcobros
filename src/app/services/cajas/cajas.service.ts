...
export class CajasService {
  ...
  updateCajaFactura(caja: any): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.put(
      `${this.base_path}${this.myApiUrl}${this.myApId}/`,
      caja,
  ...
  AddCaja(caja: any): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.post(`${this.base_path}${this.myApiUrl}/`, caja, {
      headers,
    });
  }

  updateFacturaCaja(caja: any): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.put(
      `${this.base_path}${this.myApiUrl}${this.myApIdFactura}`,
      caja,
      { headers }
    );
  }
